defmodule TradingIndicators.TestSupport.BenchmarkHelpers do
  ...
  def comparative_benchmark do
    ...
    indicators = %{
      ...
      "MACD" => fn data ->
        TradingIndicators.Trend.MACD.calculate(data,
          fast_period: 12,
          slow_period: 26,
          signal_period: 9
        )
    ...
    Enum.reduce(data_sizes, %{}, fn size, acc ->
      ...
      size_results =
        Enum.reduce(indicators, %{}, fn {name, fun}, size_acc ->
          try do
  ...
  def performance_report(indicator_fun, indicator_name) do
    ...
    # 1 second for 1k data points
    acceptable_time_1k = 1_000_000
    # 100MB
    acceptable_memory_mb = 100
    ...
    recommendations =
      if performance_1k > acceptable_time_1k do
    ...
    recommendations =
      if memory_1k && memory_1k.memory_used > acceptable_memory_mb * 1_024 * 1_024 do
  ...
  defp default_stress_scenarios do
    [
      ...
      %{
        ...
        data:
          Stream.cycle([Decimal.new("1.0"), Decimal.new("1000.0")])
          |> Enum.take(1_000)
      }
    ]
  end
  ...
  defp generate_extreme_volatility_data(size) do
    ...
    |> Enum.map(fn _i ->
      # Extreme volatility
      # +/- 25% variation
      variation = (:rand.uniform() - 0.5) * 0.5
  ...
  defp calculate_performance_rating(performance_time, memory_result) do
    ...
    time_rating =
      cond do
        # < 100ms
        performance_time < 100_000 -> 5
        # < 500ms  
        performance_time < 500_000 -> 4
        # < 1s
        performance_time < 1_000_000 -> 3
        # < 5s
        performance_time < 5_000_000 -> 2
        true -> 1
      end
    ...
    memory_rating =
      if memory_result do
        cond do
          # < 1MB
          memory_result.memory_used < 1_048_576 -> 5
          # < 10MB
          memory_result.memory_used < 10_485_760 -> 4
          # < 100MB
          memory_result.memory_used < 104_857_600 -> 3
          # < 1GB
          memory_result.memory_used < 1_073_741_824 -> 2
          true -> 1
        end
      else
        # Default if no memory data
        3
      end
