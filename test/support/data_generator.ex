defmodule TradingIndicators.TestSupport.DataGenerator do
  ...
  def sample_prices(size) when is_integer(size) and size > 0 do
    ...
    |> Enum.map_reduce(base_price, fn i, current_price ->
      ...
      change_percent = :rand.uniform() * 0.04 - 0.02  # -2% to +2%
      new_price = current_price * (1 + change_percent)
      
      # Calculate OHLC with realistic relationships
      open = current_price
      close = new_price
      {low, high} = if new_price > current_price do
  ...
  def sample_prices_with_trend(size, trend \\ :sideways) do
    base_price = 100.0
    trend_factor = case trend do
      :up -> 0.001      # 0.1% upward bias
      :down -> -0.001   # 0.1% downward bias
      :sideways -> 0    # No bias
    end
  ...
  def deterministic_sample_prices(size) when is_integer(size) and size > 0 do
    1..size
    |> Enum.map(fn i ->
      base = 100 + (i * 0.5)  # Gradual price increase
  ...
  def edge_case_data do
    [
      # Normal data
      %{open: Decimal.new("100"), high: Decimal.new("105"), low: Decimal.new("99"), 
        close: Decimal.new("103"), volume: 1000, timestamp: ~U[2024-01-01 09:30:00Z]},
        
      # Zero volume
      %{open: Decimal.new("103"), high: Decimal.new("106"), low: Decimal.new("102"), 
        close: Decimal.new("105"), volume: 0, timestamp: ~U[2024-01-01 09:31:00Z]},
        
      # Very high volume
      %{open: Decimal.new("105"), high: Decimal.new("107"), low: Decimal.new("104"), 
        close: Decimal.new("106"), volume: 1_000_000, timestamp: ~U[2024-01-01 09:32:00Z]},
        
      # Doji pattern (open = close)
      %{open: Decimal.new("106"), high: Decimal.new("108"), low: Decimal.new("105"), 
        close: Decimal.new("106"), volume: 1500, timestamp: ~U[2024-01-01 09:33:00Z]},
        
      # Gap up
      %{open: Decimal.new("110"), high: Decimal.new("112"), low: Decimal.new("109"), 
        close: Decimal.new("111"), volume: 2000, timestamp: ~U[2024-01-01 09:34:00Z]}
    ]
  end
