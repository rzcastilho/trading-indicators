defmodule TradingIndicators.TestSupport.IntegrationHelpers do
  ...
  def test_indicator_correlations(data, correlations) do
    ...
    indicator_results =
      Enum.reduce(correlations, %{}, fn {combo_name, indicators}, acc ->
        combo_results =
          Enum.reduce(indicators, %{}, fn {ind_name, {module, fun, args}}, combo_acc ->
            try do
              result = apply(module, fun, [data | args])
    ...
    correlation_analysis =
      Enum.reduce(indicator_results, %{}, fn {combo_name, combo_results}, acc ->
        # Check if all indicators succeeded
        all_successful =
          Enum.all?(combo_results, fn {_name, result} ->
