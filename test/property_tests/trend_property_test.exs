defmodule TradingIndicators.PropertyTests.TrendTest do
  ...
  describe "SMA properties" do
    ...
    property "SMA values are always finite" do
      check all prices <- PropertyGenerators.price_series(min_length: 10, max_length: 50),
                period <- PropertyGenerators.valid_period(min: 2, max: 10) do
        result = SMA.calculate(prices, period)
    ...
    property "SMA values are within reasonable bounds of input" do
      check all prices <- PropertyGenerators.price_series(min_length: 20, max_length: 50),
                period <- PropertyGenerators.valid_period(min: 5, max: 10) do
        result = SMA.calculate(prices, period)
    ...
    property "SMA is monotonic for monotonic input" do
      check all length <- integer(10..30),
                base_price <- float(min: 10.0, max: 1000.0),
                direction <- member_of([:up, :down]) do
        
        prices = case direction do
          :up -> 
            1..length 
  ...
  describe "EMA properties" do
    property "EMA converges faster than SMA to new values" do
      check all base_value <- float(min: 50.0, max: 150.0),
                jump_value <- float(min: 200.0, max: 300.0),
                period <- PropertyGenerators.valid_period(min: 5, max: 20) do
  ...
  describe "WMA properties" do
    property "WMA gives more weight to recent values" do
      check all base_value <- float(min: 50.0, max: 150.0),
                ...
                period <- PropertyGenerators.valid_period(min: 3, max: 10) do
        
        # Create series that ends with different value
        prices = List.duplicate(Decimal.from_float(base_value), period - 1) ++ 
  ...
  describe "MACD properties" do
    property "MACD signal line is EMA of MACD line" do
      check all prices <- PropertyGenerators.price_series(min_length: 50, max_length: 100),
                short_period <- PropertyGenerators.valid_period(min: 5, max: 15),
                long_period <- PropertyGenerators.valid_period(min: 20, max: 35),
                signal_period <- PropertyGenerators.valid_period(min: 5, max: 15) do
        ...
        if long > short do
          result = MACD.calculate(prices, fast_period: short, slow_period: long, signal_period: signal_period)
