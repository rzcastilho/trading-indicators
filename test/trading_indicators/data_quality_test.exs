defmodule TradingIndicators.DataQualityTest do
  ...
  describe "validate_time_series/1" do
    ...
    test "detects duplicate timestamps" do
      duplicate_data = [
        ...
        %{List.first(@valid_data) | close: Decimal.new("104.0")}
      ]
    ...
    test "detects chronological order errors" do
      out_of_order_data = [
        ...
        Enum.at(@valid_data, 1),
        # Earlier timestamp second
        Enum.at(@valid_data, 0)
      ]
      ...
      chronological_issue =
        Enum.find(report.issues, fn issue -> issue.type == :chronological_error end)
    ...
    test "detects OHLC relationship violations" do
      invalid_ohlc_data = [
        %{
          open: Decimal.new("100.0"),
          # High less than open - violation
          high: Decimal.new("95.0"),
          low: Decimal.new("99.0"),
          close: Decimal.new("103.0"),
          volume: 1000,
          timestamp: ~U[2024-01-01 09:30:00Z]
      ...
      ohlc_issue =
        Enum.find(report.issues, fn issue ->
  ...
  describe "normalize_data/3" do
    ...
    test "supports custom field for normalization" do
      assert {:ok, normalized_data} =
               DataQuality.normalize_data(@valid_data, :minmax, field: :volume)
  ...
  describe "quality_report/1" do
    ...
    test "provides quality recommendations based on issues" do
      low_quality_data = [
        ...
        # Out of order
        %{List.first(@valid_data) | timestamp: ~U[2024-01-01 09:29:00Z]},
        List.first(@valid_data)
      ]
  ...
  describe "property tests" do
    ...
    test "data quality score is between 0 and 100" do
      test_datasets = [
        ...
        [%{invalid: "data"}],
        @valid_data ++
          [%{open: "invalid", high: nil, low: nil, close: nil, volume: -1, timestamp: nil}]
      ]
  ...
  describe "edge cases" do
    test "handles data with missing OHLC fields" do
      incomplete_data = [
        ...
        %{open: Decimal.new("100.0"), volume: 1000, timestamp: ~U[2024-01-01 09:30:00Z]},
        List.first(@valid_data)
      ]
