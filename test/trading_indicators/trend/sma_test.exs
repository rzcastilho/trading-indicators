defmodule TradingIndicators.Trend.SMATest do
  ...
  describe "streaming functionality" do
    ...
    test "update_state/2 accumulates data until period is reached" do
      ...
      assert Decimal.equal?(result3.value, Decimal.new("102.0"))  # (100 + 102 + 104) / 3
    end
    ...
    test "update_state/2 maintains rolling window" do
      ...
      {:ok, state, _} = SMA.update_state(state, %{close: Decimal.new("100"), timestamp: ~U[2024-01-01 09:30:00Z]})
      {:ok, state, result} = SMA.update_state(state, %{close: Decimal.new("102"), timestamp: ~U[2024-01-01 09:31:00Z]})
      
      assert Decimal.equal?(result.value, Decimal.new("101.00"))  # (100 + 102) / 2
      
      # Add third point - should maintain window size of 2
      {:ok, state, result} = SMA.update_state(state, %{close: Decimal.new("104"), timestamp: ~U[2024-01-01 09:32:00Z]})
      
      assert length(state.prices) == 2  # Window size maintained
      assert Decimal.equal?(result.value, Decimal.new("103.00"))  # (102 + 104) / 2
    end
    ...
    test "update_state/2 handles different sources" do
      ...
      assert Decimal.equal?(result.value, Decimal.new("106.0"))  # (105 + 107) / 2
  ...
  describe "edge cases and robustness" do
    test "handles very large numbers" do
      ...
      expected = Decimal.new("1000000000.0")  # Average of the two
    ...
    test "handles very small numbers" do
      ...
      expected = Decimal.new("0.00")  # Average rounded to 2 decimal places
      assert Decimal.equal?(Enum.at(results, 0).value, expected)
    end
  ...
  describe "property-based testing" do
    ...
    test "SMA values are between min and max of contributing prices" do
      ...
      data = for {price, i} <- Enum.with_index(prices, 1) do
        %{
