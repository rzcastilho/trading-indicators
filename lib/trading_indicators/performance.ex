defmodule TradingIndicators.Performance do
  ...
  def benchmark_indicator(indicator, datasets, opts \\ []) do
    ...
    try do
      ...
      total_memory = if memory_profile do
  ...
  def benchmark_pipeline(pipeline_config, datasets, opts \\ []) do
    ...
    try do
      ...
      benchmark_result = %{
        ...
        execution_results: Enum.map(results, fn {result, time} -> {result.execution_metrics, time} end),
        timestamp: DateTime.utc_now()
      }
  ...
  def memory_profile(_operation) do
    {:error, %Errors.InvalidParams{message: "Operation must be a function with arity 0", param: :operation}}
  end
  ...
  def cache_stats do
    case :ets.whereis(@cache_table) do
      ...
      _ ->
        config = case :ets.lookup(@cache_table, :config) do
          [{:config, config}] -> config
          [] -> %{enabled: false}
        end

        stats = case :ets.lookup(@cache_table, :stats) do
          [{:stats, stats}] -> stats
          [] -> %{hits: 0, misses: 0}
        end

        cache_size = :ets.info(@cache_table, :size) - 2  # Subtract config and stats entries
  ...
  defp benchmark_dataset(indicator, dataset, params, iterations, memory_profile) do
    ...
    memory_usage = if memory_profile do
      {:ok, profile} = memory_profile(fn -> 
  ...
  defp calculate_throughput(datasets, total_time) do
    ...
    if total_time > 0 do
      total_points * 1_000_000 / total_time  # points per second
    else
      0.0
    end
  end
  ...
  defp analyze_memory_usage(benchmark_results) do
    high_memory_results = Enum.filter(benchmark_results, fn result ->
      memory_per_point = case Map.get(result, :memory_usage, 0) do
        0 -> 0
        memory -> memory / Enum.sum(Map.get(result, :dataset_sizes, [1]))
      end
      memory_per_point > 1000  # More than 1KB per data point
    end)

    if length(high_memory_results) > 0 do
      [%{
        type: :memory,
        description: "High memory usage detected. Consider implementing streaming mode or data chunking.",
        impact: :high,
  ...
  defp analyze_cpu_usage(benchmark_results) do
    slow_results = Enum.filter(benchmark_results, fn result ->
      avg_time = Map.get(result, :average_time, 0)
      avg_time > 100_000  # More than 100ms average
    end)

    if length(slow_results) > 0 do
      [%{
        type: :cpu,
        description: "Slow execution detected. Consider algorithmic optimizations or parallel processing.",
        impact: :high,
