defmodule TradingIndicators.Security do
  ...
  defp validate_memory_safety(data) do
    ...
    max_size = Application.get_env(:trading_indicators, :max_data_size, 50_000_000) # 50MB
  ...
  defp validate_map_values(map) do
    dangerous_values = Map.values(map)
  ...
  defp validate_string_content(string) do
    dangerous_patterns = [
      ...
      ~r/data:/i,         # Data protocol
      ~r/eval\(/i,        # Code evaluation
      ~r/exec\(/i,        # Code execution
  ...
  defp validate_custom_parameters(params) do
    ...
    dangerous_keys = Map.keys(params)
  ...
  defp validate_ohlcv_structure(ohlcv_data) do
    ...
    invalid_bars = Enum.with_index(ohlcv_data)
  ...
  defp validate_timestamp_security(ohlcv_data) do
    ...
    future_limit = DateTime.add(now, 86400) # 1 day in future
    past_limit = DateTime.add(now, -31_536_000) # 1 year in past
    
    invalid_timestamps = Enum.with_index(ohlcv_data)
    |> Enum.filter(fn {bar, _index} ->
